...
describe("psy-vesting closeVestingContract", () => {
  ...
  describe("Vesting schedule has been completed and claimed", () => {
    ...
    it("should transfer the lamports to the desired address", async () => {
      const issuerAcctInfoBefore = await provider.connection.getAccountInfo(
      ...
      const contractAcctAfter = await provider.connection.getAccountInfo(
        vestingContractKeypair.publicKey
      );
      assert.ok(!contractAcctAfter);

      // Validate the issuer got the lamports
      const issuerAcctInfo = await provider.connection.getAccountInfo(
        program.provider.wallet.publicKey
      );
      if (!issuerAcctInfo || !issuerAcctInfoBefore) {
        throw new Error("Cannot load issuer account info");
      }
      expect(issuerAcctInfo.lamports).to.greaterThan(
        issuerAcctInfoBefore.lamports
      );

      // test that the token vault gets closed
      const tokenVault = await provider.connection.getAccountInfo(
        vestingContract.tokenVault
      );
