...
describe("psy-vesting createVestingContract", () => {
  ...
  describe("Given a valid SPL Token Mint and vesting information", () => {
    ...
    it("should create a valid VestingContract", async () => {
      ...
      assert.equal(
        ...
        new anchor.BN(10_000_000)
          .mul(new anchor.BN(10).pow(new anchor.BN(tokenMintInfo.decimals)))
          .toString()
      );

      // make rpc call to create the VestingContract
      try {
        ({ tokenVaultKey, vestingContractKeypair } =
      ...
      let total = vestingSchedule.reduce(
        (acc, curr) => curr.amount.add(acc),
        new anchor.BN(0)
      );
    ...
    describe("no update authority", () => {
      it("should not store the update authority", async () => {
        ...
        assert.ok(
          vestingContract.updateAuthority.equals(SystemProgram.programId)
        );
      });
    });
  ...
  describe("Destination's mint address does not match token mint", () => {
    ...
    beforeEach(async () => {
      ...
      badeDestAddr = await newToken.createAssociatedTokenAccount(
        payer.publicKey
      );
    });
